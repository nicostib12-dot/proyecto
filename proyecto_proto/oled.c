...
void OLED_Init(void) {
    ...
    OLED_Command(0x20); OLED_Command(0x02); // ? 0x02 = modo P�GINA
    OLED_Command(0xA1);
    OLED_Command(0xC8);
...
// OLED_Data: enviar m�ltiples bytes en una sola transacci�n ??
// Usar funciones primitivas I2C directamente en ClearLine y WriteChar
...
void OLED_ClearLine(unsigned char linea) {
    unsigned char i;
    // Posicionar cursor al inicio de la l�nea
    OLED_Command(0xB0 + linea);
    OLED_Command(0x02);  // col baja = 2 (offset)
    OLED_Command(0x10);  // col alta = 0
    // Enviar 128 ceros en UNA sola transacci�n I2C
    I2C_Start();
    I2C_Write(OLED_ADDR);
    I2C_Write(0x40);      // modo datos, stream
    for (i = 0; i < 128; i++) I2C_Write(0x00);
    I2C_Stop();
}
...
void OLED_SetCursor(unsigned char col, unsigned char page) {
    col += 2; // offset de hardware: columnas visibles empiezan en 2
    OLED_Command(0xB0 + page);
...
void OLED_WriteChar(char c) {
    ...
    // 5 columnas del car�cter + 1 de separaci�n en UNA transacci�n
    I2C_Start();
    I2C_Write(OLED_ADDR);
    I2C_Write(0x40);
    for (i = 0; i < 5; i++) I2C_Write(font5x7[index][i]);
    I2C_Write(0x00);      // separaci�n
    I2C_Stop();
}
